...
def check_user(message: types.Message) -> bool:
    ...
    query = f"SELECT telegram_id FROM handy-digit-312214.TG_Bot_Stager.users WHERE telegram_id = {tg_id}"
    ...
    if check_user(message):
        try:
            df = pd.DataFrame({"telegram_id": [message.from_user.id],
                               ...
                               "is_confirmed": [True],
                               "regiestred_at": [bytes(str(message.date.date()), "utf-8")]})
            df.to_gbq("handy-digit-312214.TG_Bot_Stager.users", project_id=PROJECT, credentials=credentials,
                      if_exists="append")
            return True
        except (Exception, ):
            return False
